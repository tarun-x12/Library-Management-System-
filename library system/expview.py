...
def View(): 
    ...
    headingLabel = Label(headingFrame1, text="View Books", bg='white', fg='purple', font = ('Courier',30,'bold'))
    ...
    Label(labelFrame, text="%-10s%-40s%-30s%-30s%-20s"%('BID','Title','Author','Status','Issued To'),bg='black',fg='white').place(relx=0.07,rely=0.1)
    Label(labelFrame, text = "--------------------------------------------------------------------------------------------------",bg='black',fg='white').place (relx=0.05,rely=0.2)

    getBooks = "select * from"+bookTable
    memname="select memname from"+bookTable
    #try:
    ...
    for i in cur:
        ...
        if i[3]=="issued":
            Label(labelFrame,text=i[4],bg='black', fg='white').place(relx=0.65,rely=y)
